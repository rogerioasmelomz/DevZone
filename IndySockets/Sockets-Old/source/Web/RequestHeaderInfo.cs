...
namespace Indy.Sockets.Web
{
	public class RequestHeaderInfo: EntityHeaderInfo
	{
	  ...
	  protected override void DoProcessHeaders()
	  {
	    ...
	    string RangeDecode = _RawHeaders.Values("Range");
	    if (RangeDecode != "")
	    {
	      Core.Global.Fetch(ref RangeDecode, "=");
        _ContentRangeStart = Core.Global.StrToInt32Def(Core.Global.Fetch(ref RangeDecode, "-"), 0);
        _ContentRangeEnd = Core.Global.StrToInt32Def(Core.Global.Fetch(ref RangeDecode), 0);
	    }

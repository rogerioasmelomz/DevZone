...
namespace Indy.Sockets.Web {
	public class EntityHeaderInfo: IDisposable {
		...
		protected virtual void DoProcessHeaders() {
			...
			_ContentLength = Core.Global.StrToInt32Def(_RawHeaders.Values("Content-Length").Trim(), -1);
			...
			if (Core.Global.StrToInt32Def(_RawHeaders.Values("Expires"), -1) != -1) {
		...
		protected virtual void DoSetHeaders() {
			...
			if (_CustomHeaders.Count > 0) {
				_RawHeaders.Text += _CustomHeaders.Text;
			}
		}

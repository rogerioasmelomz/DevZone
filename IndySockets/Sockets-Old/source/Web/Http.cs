...
namespace Indy.Sockets.Web {
	public sealed class Http {
		...
		public static readonly string HttpServer_ServerSoftware = "Indy.NET/1.0"; // add real version
		...
		private static short ParseDayOfMonth(ref string AValue, string ADelim) {
			short Dt = Core.Global.StrToInt16Def(Core.Global.Fetch(ref AValue, ADelim), 1);
			AValue = AValue.TrimStart();
			return Dt;
		}

		private static short ParseMonth(ref string AValue, string ADelim) {
			short Mo = StrToMonth(Core.Global.Fetch(ref AValue, ADelim));
			AValue = AValue.TrimStart();
			return Mo;
		}
		...
		private static DateTime RawStrInternetToDateTime(ref string AValue) {
			...
			try {
				...
				if (StrToMonth(Core.Global.Fetch(ref AValue, ADelim, false)) > 0) {
				...
				sTime = Core.Global.Fetch(ref AValue);
				Yr = Core.Global.StrToInt16Def(sTime, 1900);
				if (Yr == 1900) {
					Yr = Core.Global.StrToInt16Def(AValue, 1900);
					AValue = sTime;
				}
				if (Yr < 80) {
					Yr += 2000;
				...
				if (AValue.IndexOf("AM") > -1) {
					LAM = true;
					Core.Global.Fetch(ref AValue, "AM");
				}
				if (AValue.IndexOf("PM") > -1) {
					LPM = true;
					Core.Global.Fetch(ref AValue, "PM");
				}
				i = AValue.IndexOf(':');
				if (i > -1) {
					sTime = Core.Global.Fetch(ref AValue, " ");
					Ho = Core.Global.StrToInt16Def(Core.Global.Fetch(ref sTime, ":"), 0);
					Min = Core.Global.StrToInt16Def(Core.Global.Fetch(ref sTime, ":"), 0);
					Sec = Core.Global.StrToInt16Def(Core.Global.Fetch(ref sTime), 0);
					AValue = AValue.TrimStart();
					if (LAM) {
						if (Ho == 12) {
							Ho = 0;

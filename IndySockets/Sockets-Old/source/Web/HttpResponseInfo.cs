...
namespace Indy.Sockets.Web
{
	public class HttpResponseInfo: ResponseHeaderInfo
	{
	  ...
	  protected CustomHttpServer _HttpServer;
    ...
    public HttpResponseInfo(TcpConnection<int, ReplyRFC> AConnection, CustomHttpServer AServer)
	  {
...
//      _RemoteConnection.Socket.WriteBufferOpen();
	    try
	    {
        _RemoteConnection.Socket.WriteLn("HTTP/1.1 " + ResponseNo.ToString() + " " + ResponseText);
	      foreach(string s in RawHeaders)
	      {
          _RemoteConnection.Socket.WriteLn(s);
	      }
	      foreach(ServerCookie sc in Cookies)
	      {
          _RemoteConnection.Socket.WriteLn("Set-Cookie: " + sc.ServerCookie);
	      }
        _RemoteConnection.Socket.WriteLn("");
	    }
	    finally
	    {
//        _RemoteConnection.Socket.WriteBufferClose();
	  ...
	  public void WriteContent()
	  {
		...
		public virtual long ServeFile(ContextRFC AContext, string AFile)
	  {
      ...
      return ContentLength;
		...
		public virtual long SmartServeFile(ContextRFC AContext, HttpRequestInfo ARequestInfo, string AFile)
	  {
      ...
      {
        return ServeFile(AContext, AFile);
      }
	  }
    ...
    public CustomHttpServer HttpServer
    {
      get
      {
        return _HttpServer;

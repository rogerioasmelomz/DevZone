...
namespace Indy.Sockets.Core {
	...
	public class CSharpTextDataBlockReader: SocketSimulator.IDataBlockReader {
		...
		public byte[] ReadBlock() {
					...
					if (BlockString[0] != '\\') // single backslash
          {
						...
						if (BlockString.Length > 1) {
							switch (BlockString[1]) {
								...
								case 'h': {
										if (BlockString.Length > 5) {
											ms.WriteByte(Byte.Parse(BlockString.Substring(2, 2), NumberStyles.HexNumber));
											BlockString = BlockString.Remove(0, 2);
										}
										BlockString = BlockString.Remove(0, 2);
										break;
							...
							BlockString = BlockString.Remove(0, 1);
						}
					}
				}
				return (ms.ToArray());
		...
		public string GetPreviewOfBlock(byte[] data) {
			return TextWriterIntercept.BytesToBeautyString(data, Encoding.ASCII);
		}
	}
}
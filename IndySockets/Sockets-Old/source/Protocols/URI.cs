...
namespace Indy.Sockets.Protocols
{
  ...
  public class URIException: Core.IndyException {
    ...
    public static string URLDecode(string ASrc)
    {
      ...
      while (i < ASrc.Length)
      {
        ...
        else
        {
          ...
          i++;
          CharCode = Core.Global.StrToInt32Def("$" + Esc, -1);
          if (CharCode != -1)
          {
...
///        _IPVersion = IPVersionEnum.v4;
        ...
        if (LTokenPos > -1)
        {
          ...
          {
            _Host = Core.Global.Fetch(ref LBuffer, "]");
            Core.Global.Fetch(ref _Host, "[");
            Core.Global.Fetch(ref LBuffer, ":");
//            _IPVersion = IPVersionEnum.v6;
          }
          else
          {
            _Host = Core.Global.Fetch(ref LBuffer, ":", true);
          }
          _Port = LBuffer;
          ...
          _Document.Remove(0, LTokenPos + 1);
          
          _Bookmark = _Document;
          _Document = Core.Global.Fetch(ref _Bookmark, "#");
        }
        else
        {
          LParamsPos = LURI.IndexOf('?');
        ...
        {
          _Bookmark = _Params;
          _Params = Core.Global.Fetch(ref _Bookmark, "#");
        }
      }
    }
